...
- (void)viewDidLoad
{
    ...
    if ([self.myPlacemark isKindOfClass:[CLPlacemark class]]) {
        NSString *addressString = CFBridgingRelease(CFBridgingRetain(ABCreateStringWithAddressDictionary(self.myPlacemark.addressDictionary, NO)));
        
        CLLocationDegrees latitude = self.myPlacemark.location.coordinate.latitude;
        CLLocationDegrees longitude = self.myPlacemark.location.coordinate.longitude;
        NSDictionary *dict = [wp createDict:addressString lat:latitude lng:longitude];
        self.location = dict;
